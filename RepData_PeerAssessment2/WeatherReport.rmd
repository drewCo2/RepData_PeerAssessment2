...
1. Remove all rows from the data set that don't include any damage, fatalities or injuries.  This will reduce the associated number of levels from the EVTYPE factor.
...
2. Whitespace is stripped from all of the event labels, and all labels are converted to uppercase.
```{r echo=TRUE, cache=TRUE}
ev<-factor(toupper(trimws(stormData$EVTYPE)))
stormData$EVTYPE = ev
```

3. Next we take the top 50 most frequent event types by occurance.  The goal is to capture a significant portion of event types so that infrequent events may be excluded without skewing the results too much.

```{r echo=TRUE, cache=TRUE}

freqs <- arrange(data.frame(table(stormData$EVTYPE)), desc(Freq))[1:50, ]
top50Prop<- sum(freqs["Freq"]) / nrow(stormData)

```

These account for `r (top50props * 100)`% of the total observations, which should not effect the final results too much if the others are omitted.  However, it is possible that a single event (not included here) could account for a large amount of damage or injuries, so I think a bit more analysis is required.
  
Our next step will be to look at the events that fall outside of the top 50, and compare their fatalities, injuries, and damage with those items in the top 50.
...
There are `r nrow(match)` observations with a total of `r tFatal` fatalities, `r tInjur` injuries and `r tDmg` in damages.  Compared to the entire data set, these observations account for `r fProp`%, `r iProp'% and `r dProp` respectively.  Therefore I conclude that it is safe to exclude these factors when completing our cleanup of the EVTYPE factor.


4. As a final measure, we will observe the top 50 labels of the EVTYPE factor and use this information to make further corrections.  This part of the process is based on reading the labels, and using grep / insight to make informed decisions about manual corrections.
...
newEv[newEv == 'TSTM WIND'] = 'THUNDERSTORM WIND'
...
```{r echo=TRUE, cache=TRUE, fig.width=8, fig.height=4, fig.cap="The Top 5 most harmful weather events, by type."}
